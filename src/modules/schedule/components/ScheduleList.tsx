    ...
    <div className={`grid ${mainPage ? "grid-cols-1" : "grid-cols-2"} gap-8`}>
      ...
      >
        {(mainPage ? startWeekDays : firstColumnDays).map((day, index) => (
          <li
            key={index}
            className="bg-blue-100 p-6 dark:bg-dark-schedule-background"
          >
            ...
            <ul className="flex flex-col gap-3">
              {day.events.map((event, idx) => (
                <li key={idx} className="ml-5">
                  <div className="font-inter-600 text-sm font-semibold text-violet-300 dark:text-dark-date">
                    {event.time}
                  </div>

                  <div className="">
                    <div className="ml-2">{event.description}</div>
                    <div className="ml-2 italic">{event.moreInfo}</div>
                  </div>
                </li>
              ))}
            </ul>
